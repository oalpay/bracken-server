...
class Boot extends Loggable {
  def boot {
    ...
    MongoAuth.siteName.default.set("BrackenApp")
    ...
    LiftRules.addToPackages("brackenapp")
    ...
    // Rest
    LiftRules.dispatch.append(brackenapp.rest.ImageService)
	LiftRules.dispatch.append(brackenapp.rest.BrackenService)

	//Build rewrites
/*	LiftRules.statelessRewrite.append {
	  case RewriteRequest(ParsePath(List("pages", pageId), _, _, _), _, _) => RewriteResponse("page" :: Nil, Map("pageId" -> pageId))
	}*/
